...
function prendi_tutti_spazio()
{
    ...
    $s2 = ['Posizione' => 2000, 'Nome' => 'Sala Conferenze', 'Descrizione' => 'Una grande sala per conferenze', 'Tipo' => 'Conferenza', 'N_tavoli' => 20];

    $spazio->nuovo(1000, 'Sala Conferenze', 'Una grande sala per conferenze', 'Conferenza', 20);
    $spazio->nuovo(2000, 'Sala Conferenze', 'Una grande sala per conferenze', 'Conferenza', 20);
    $spazi = $spazio->prendi_tutti();

    if (!$spazi || count($spazi) < 2 || !contiene($spazi, $s1) || !contiene($spazi, $s2)) {
        return False;
    }
